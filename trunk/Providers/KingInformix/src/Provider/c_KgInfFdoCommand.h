...
#ifndef _c_KgInfFdoCommand_h
#define _c_KgInfFdoCommand_h


template <class FDO_COMMAND> class c_KgInfFdoCommand : public FDO_COMMAND
{
    friend class CONNECTION_CLASS;

protected:
    /// The connection this command was created from.
    FdoPtr<c_KgInfConnection> m_Connection;
...
protected:
    ...
    c_KgInfFdoCommand (c_KgInfConnection* Connection) : m_Connection (Connection)
    {
        FDO_SAFE_ADDREF (m_Connection.p);
    }

    /// Default destructor for a command.
    virtual ~c_KgInfFdoCommand() 
    { 
    }
...
public:
    ...
    virtual void SetConnection (c_KgInfConnection* Connection)
    {
...
FdoITransaction* c_KgInfFdoCommand<FDO_COMMAND>::GetTransaction ()
{
    return (NULL);
}
...
// Sets the transaction in which the command executes.
template <class FDO_COMMAND> 
void c_KgInfFdoCommand<FDO_COMMAND>::SetTransaction (FdoITransaction* value)
{
...
// executed multiple times.
template <class FDO_COMMAND> 
void c_KgInfFdoCommand<FDO_COMMAND>::Prepare ()
{
}
...
// not supported, no exception is generated.
template <class FDO_COMMAND> 
void c_KgInfFdoCommand<FDO_COMMAND>::Cancel ()
{
}
...
// this collection. 
template <class FDO_COMMAND> 
FdoParameterValueCollection* c_KgInfFdoCommand<FDO_COMMAND>::GetParameterValues()
{
  if( !m_Parameters.p )
  {
