...
#ifndef _c_KgMssFdoCommand_h
#define _c_KgMssFdoCommand_h


template <class FDO_COMMAND> class c_KgMssFdoCommand : public FDO_COMMAND
{
    friend class CONNECTION_CLASS;

protected:
    /// The connection this command was created from.
    FdoPtr<c_KgMssConnection> m_Connection;
...
protected:
    ...
    c_KgMssFdoCommand (c_KgMssConnection* Connection) : m_Connection (Connection)
    {
        FDO_SAFE_ADDREF (m_Connection.p);
    }

    /// Default destructor for a command.
    virtual ~c_KgMssFdoCommand() 
    { 
    }
...
public:
    ...
    virtual void SetConnection (c_KgMssConnection* Connection)
    {
...
FdoITransaction* c_KgMssFdoCommand<FDO_COMMAND>::GetTransaction ()
{
    return (NULL);
}
...
// Sets the transaction in which the command executes.
template <class FDO_COMMAND> 
void c_KgMssFdoCommand<FDO_COMMAND>::SetTransaction (FdoITransaction* value)
{
...
// executed multiple times.
template <class FDO_COMMAND> 
void c_KgMssFdoCommand<FDO_COMMAND>::Prepare ()
{
}
...
// not supported, no exception is generated.
template <class FDO_COMMAND> 
void c_KgMssFdoCommand<FDO_COMMAND>::Cancel ()
{
}
...
// this collection. 
template <class FDO_COMMAND> 
FdoParameterValueCollection* c_KgMssFdoCommand<FDO_COMMAND>::GetParameterValues()
{
  if( !m_Parameters.p )
  {
