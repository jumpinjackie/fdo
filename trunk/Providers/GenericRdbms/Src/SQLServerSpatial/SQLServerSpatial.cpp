 ...
 *
 */
// SqlServer.cpp : Defines the entry point for the DLL application.
//
#include "stdafx.h"
#ifdef _WIN32

#include "Fdo/FdoRdbmsSqlServerConnection.h"
#include "FdoRdbmsUtil.h"
#include "Fdo/FdoRdbmsSqlServerProviderInfo.h"
#include <olectl.h>
...
HRESULT APIENTRY DllRegisterServer (void)
{
    ...
    if (registry != NULL)
    {
        registry->RegisterProvider (RDBMS_SQLSERVER_PROVIDER_NAME,
                                    NlsMsgGet(FDORDBMS_474, RDBMS_SQLSERVER_PROVIDER_DEFAULT_DISPLAY_NAME),
                                    NlsMsgGet(FDORDBMS_475, RDBMS_SQLSERVER_PROVIDER_DEFAULT_DESCRIPTION),
                                    RDBMS_SQLSERVER_PROVIDER_VERSION,
                                    RDBMS_SQLSERVER_FDO_VERSION,
                                    module,
                                    false);
        ret = S_OK;
    }
...
HRESULT APIENTRY DllUnregisterServer (void)
{
    ...
    if (registry != NULL)
    {
        registry->UnregisterProvider (RDBMS_SQLSERVER_PROVIDER_NAME);
        ret = S_OK;
    }
...
// There is no Linux shared library for SqlServer. Also, there supporting Linux is not required. 
// However, keep this block here for future enhancements.
const char *RDBMS_LIB_NAME = "????.so";
#include <link.h>
#include <wchar.h>
#include <inc/ut/thread.h>

static char com_dir[PATH_MAX];
...
{
   return FdoRdbmsSqlServerConnection::Create();
}