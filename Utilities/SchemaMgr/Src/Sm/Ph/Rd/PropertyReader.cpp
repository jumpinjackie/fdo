            ...
            if ( !IsEOF() ) {
                if ( mFieldIdx < mDbObject->GetColumns()->GetCount() ) {
                    ...
                    if ( row && CheckColumn(column) ) {
                        ...
                        field = row->GetFields()->GetItem(L"tablename");
                        field->SetFieldValue( mDbObject->GetName() );

                        field = row->GetFields()->GetItem(L"columntype");
                        ...
                        field = row->GetFields()->GetItem(L"columnsize");
                        field->SetFieldValue( FdoStringP::Format(L"%d", column ? column->GetLength() : 0) );

                        field = row->GetFields()->GetItem(L"columnscale");
                        field->SetFieldValue( FdoStringP::Format(L"%d", column ? column->GetScale() : 0) );

                        field = row->GetFields()->GetItem(L"isnullable");
                        field->SetFieldValue( column->GetNullable() ? L"1" : L"0" );

                        field = row->GetFields()->GetItem(L"isfixedcolumn");
                        field->SetFieldValue( L"1" );

                        field = row->GetFields()->GetItem(L"idposition");
                        field->SetFieldValue( GetIdPosn(column->GetName()) );

                        field = row->GetFields()->GetItem(L"isfeatid");
                        field->SetFieldValue( L"0" );

                        field = row->GetFields()->GetItem(L"isautogenerated");
