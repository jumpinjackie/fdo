...
void FdoSmLpClassBase::CreateUkeys()
{
				...
				if ( pMatchedProp )
					...
					GetErrors()->Add( FdoSmErrorType_Other, 
						FdoSchemaException::Create(
								FdoSmError::NLSGetMessage(
									FDO_NLSID(FDOSM_410), 
									(FdoString*) pUniqueCol->GetName(), 
									(FdoString*) pUniqueCol->GetQName() )
							)
						);
 			}
    ...
    if ( phTable && count == 0 ) {
		...
		for ( int i = 0; i < mUniqueConstraints->GetCount(); i++ ) {
			...
			// Create a new set of unique keys and add it to the collection of collections. 
			FdoSmPhColumnsP				ukeyColumns = new FdoSmPhColumnCollection();

			ukeysPhColumnsColl->Add( ukeyColumns );

			for ( int j = 0; j < pProps->GetCount(); j++ ) {
				FdoSmLpDataPropertyP pProp = pProps->GetItem(j)->SmartCast<FdoSmLpDataPropertyDefinition>(true);

				if ( pProp ) {
					FdoSmPhColumnP pColumn = pProp->GetColumn();

					if ( pColumn ) {
  						phTable->AddUkeyCol( i, pColumn->GetName());
					}
				}
			}
		}
			...
			if ( pMatchedProp ) { 
				...
				GetErrors()->Add( FdoSmErrorType_Other, 
					FdoSchemaException::Create(
							FdoSmError::NLSGetMessage(
								FDO_NLSID(FDOSM_411), 
								(FdoString*) phCkey->GetColumnName(), 
								(FdoString*) phCkey->GetColumnName() )
							)
					);
		}
