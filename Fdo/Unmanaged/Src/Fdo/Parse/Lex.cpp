...
FdoLex::FdoLex(FdoParse *pParse, FdoString* str)
{
	m_line = str;
    ...
    pParse->fdo_expression_yylineno = 1;
}  
...
FdoInt32 FdoLex::GetToken(FdoParse *pParse) 
{
	...
	FdoInt32	column = pParse->fdo_expression_yylineno;
        ...
        if (iswalpha(m_ch)) 
		{
            ...
            if ((tktmp = FindKeyWord(wordOnStack, g_aKeyWords, KEYWORDCOUNT)) != FdoToken_Undefined)
			{
                ...
                if (m_token == FdoToken_DATE) 
				{
                    ...
                    if (m_ch == '\'') 
					{
						column = pParse->fdo_expression_yylineno;
						...
						else 
						{
							...
							throw FdoException::Create(FdoException::NLSGetMessage(FDO_NLSID(PARSE_6_MISSINGQUOTE_d), column));
						}
                    }
...
const wchar_t FdoLex::if_getch(FdoParse *pParse) 
{
	wchar_t	c;
	...
	{
		c = m_line[m_cc++];
		pParse->fdo_expression_yylineno++;
		if (c == '\n' || c == '\r') 
			c = ' ';
	}
	return c;
...
void FdoLex::getword(FdoParse *pParse, wchar_t* pstr, FdoInt32 maxCharacters)
{
...
// TODO: range checking
wchar_t* FdoLex::getdigits(FdoParse *pParse, wchar_t* pstr)
{
	while (iswdigit(m_ch)) 
	{
		*pstr = m_ch;
...
// Get unsigned number
FdoUInt32 FdoLex::get_unsigned(FdoParse *pParse)
{
...
//  Get number
bool FdoLex::getnumber(FdoParse *pParse, bool sign)
{
    ...
    if (towupper(m_ch) == 'E') 
	{
		...
		*pstr = m_ch;
        ...
        if (!iswdigit(m_ch)) 
		{
            ...
            throw FdoException::Create(FdoException::NLSGetMessage(FDO_NLSID(PARSE_1_INVALIDDIGIT)));
        }
        pstr = getdigits(pParse, pstr);
		*pstr = CHR_NULL;
#ifdef _WIN32
...
)
{
	...
	*y = (FdoUInt16) store;
	...
	if (store < 1 || store > 12) 
	{
		...
		throw FdoException::Create(FdoException::NLSGetMessage(FDO_NLSID(PARSE_3_DATETIMEOUTOFRANGE)));
	}
	if (m_ch != '-') 
	{
		// Error !!! Wrong datetime string format
...
//  Get second value
bool FdoLex::get_second(FdoParse *pParse, double *sf)
{
