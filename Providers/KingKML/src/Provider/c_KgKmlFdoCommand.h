...
#ifndef _c_KgKmlFdoCommand_h
#define _c_KgKmlFdoCommand_h


template <class FDO_COMMAND> class c_KgKmlFdoCommand : public FDO_COMMAND
{
    friend class CONNECTION_CLASS;

protected:
    /// The connection this command was created from.
    FdoPtr<c_KgKmlConnection> m_Connection;
...
protected:
    ...
    c_KgKmlFdoCommand (c_KgKmlConnection* Connection) : m_Connection (Connection)
    {
        FDO_SAFE_ADDREF (m_Connection.p);
    }

    /// Default destructor for a command.
    virtual ~c_KgKmlFdoCommand() 
    { 
    }
...
public:
    ...
    virtual void SetConnection (c_KgKmlConnection* Connection)
    {
...
FdoITransaction* c_KgKmlFdoCommand<FDO_COMMAND>::GetTransaction ()
{
    return (NULL);
}
...
// Sets the transaction in which the command executes.
template <class FDO_COMMAND> 
void c_KgKmlFdoCommand<FDO_COMMAND>::SetTransaction (FdoITransaction* value)
{
...
// executed multiple times.
template <class FDO_COMMAND> 
void c_KgKmlFdoCommand<FDO_COMMAND>::Prepare ()
{
}
...
// not supported, no exception is generated.
template <class FDO_COMMAND> 
void c_KgKmlFdoCommand<FDO_COMMAND>::Cancel ()
{
}
...
// this collection. 
template <class FDO_COMMAND> 
FdoParameterValueCollection* c_KgKmlFdoCommand<FDO_COMMAND>::GetParameterValues()
{
  if( !m_Parameters.p )
  {
