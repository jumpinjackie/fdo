#include "Pch.h"
#include <cmath>
#include "InsertUpdateDeleteTests.h"

#pragma warning(disable : 4996)

CPPUNIT_TEST_SUITE_REGISTRATION(InsertUpdateDeleteTests);
CPPUNIT_TEST_SUITE_NAMED_REGISTRATION(InsertUpdateDeleteTests, "InsertUpdateDeleteTests");

InsertUpdateDeleteTests::InsertUpdateDeleteTests(void)
{
}
...
void InsertUpdateDeleteTests::PrepareTables(FdoIConnection *Connection)
{
...
void InsertUpdateDeleteTests::Insert_BadClassName()
{
    ...
    try 
    {
        insert->SetFeatureClassName(L"IDontExist");
        insert->Execute();
        conn->Close();
        CPPUNIT_FAIL("This should thrown on bad/non-existent class name");
    }
    catch (FdoException* ex)
    {
        ...
        conn->Close();
        FDO_SAFE_RELEASE(ex);
        CPPUNIT_ASSERT(msg == L"FDO class 'IDontExist' not found in schema.");
    }
}

void InsertUpdateDeleteTests::Update_BadClassName()
{
    FdoPtr<FdoIConnection> conn = UnitTestUtil::CreateDefaultConnection();
    ASSERT_CAN_CONNECT(conn)
    ...
    try 
    {
        update->SetFeatureClassName(L"IDontExist");
        update->Execute();
        conn->Close();
        CPPUNIT_FAIL("This should thrown on bad/non-existent class name");
    }
...
void InsertUpdateDeleteTests::Delete_BadClassName()
{
    FdoPtr<FdoIConnection> conn = UnitTestUtil::CreateDefaultConnection();
    ASSERT_CAN_CONNECT(conn)
    ...
    try 
    {
        deleteCmd->SetFeatureClassName(L"IDontExist");
        deleteCmd->Execute();
        conn->Close();
        CPPUNIT_FAIL("This should thrown on bad/non-existent class name");
    }
...
//
void InsertUpdateDeleteTests::InsertUpdateDelete()
{
    FdoPtr<FdoIConnection> conn = UnitTestUtil::CreateDefaultConnection();
    ASSERT_CAN_CONNECT(conn)
    ...
    try
    {
        FdoStringP src_classname = UnitTestUtil::GetFeatureClass(L"UT_IUD", L"GEOM");
        ...
        // execute update
        {
            ...
            if (updated != 1)
            {
                CPPUNIT_FAIL("InsertUpdateDeleteTests::InsertUpdateDelete: Update should returned 1 for num of changed rows");
            }
        }
        // execute delete
        if (false)
        {
            ...
            if (deleted != 1)
            {
                CPPUNIT_FAIL("InsertUpdateDeleteTests::InsertUpdateDelete: Delete should returned 1 for num of changed rows");
            }
        }
...
} //end of InsertUpdateDeleteTests::InsertUpdateDelete()

void InsertUpdateDeleteTests::InsertCurvedPolygon()
{
...
// Test geometry transform from sdo_geometry to FDF fromat and back to sdo_geometry
//
void InsertUpdateDeleteTests::InsertUnicode()
{
#ifndef _WIN32
    #warning FIXME: We currently cannot insert unicode strings on Linux
#endif
    FdoPtr<FdoIConnection> conn = UnitTestUtil::CreateDefaultConnection();
    ASSERT_CAN_CONNECT(conn)
    ...
    try
    {
        ...
        // Execute insert
        {
            ...
            // Name string property
            FdoPtr<FdoStringValue> name_val = (FdoStringValue *)FdoDataValue::Create(L"Name1 ���� �Ў��");
            FdoPtr<FdoPropertyValue> propval_name = FdoPropertyValue::Create(L"NAME", name_val);
        ...
        // execute query
        {
            ...
            while (reader->ReadNext())
            {
                ...
                if (name.ICompare(L"Name1 ���� �Ў��") != 0)
                {
                    std::string msg = "InsertUpdateDeleteTests::InsertUnicode: Not valid NAME column data. Got: '";
                    msg += (const char*)name;
                    msg += "'";
                    CPPUNIT_FAIL(msg);
                }
...
} //end of InsertUpdateDeleteTests::InsertUnicode

//
// Test geometry transform from sdo_geometry to FDF fromat and back to sdo_geometry
//
void InsertUpdateDeleteTests::InsertMultiPoint()
{
    ...
    conn->Close();
    conn = UnitTestUtil::CreateDefaultConnection();
    ASSERT_CAN_CONNECT(conn)
...
// Test geometry transform from sdo_geometry to FDF fromat and back to sdo_geometry
//
void InsertUpdateDeleteTests::InsertMultiGeometry()
{
    FdoPtr<FdoIConnection> conn = UnitTestUtil::CreateDefaultConnection();
    ASSERT_CAN_CONNECT(conn)
...
} //end of InsertUpdateDeleteTests::InsertMultiGeometry()

/*
void InsertUpdateDeleteTests::Update_BLDG_POLYGON()
{
...
try
{
  FdoStringP src_classname = UnitTestUtil::GetFeatureClass(L"BLDG_POLYGON", L"GEOMETRY");
