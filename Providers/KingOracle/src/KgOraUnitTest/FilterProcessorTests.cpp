#include "Pch.h"
#include "FilterProcessorTests.h"
#include "UnitTestUtil.h"
#include "TestCommonMiscUtil.h"

CPPUNIT_TEST_SUITE_REGISTRATION(FilterProcessorTests);
CPPUNIT_TEST_SUITE_NAMED_REGISTRATION(FilterProcessorTests, "FilterProcessorTests");

FilterProcessorTests::FilterProcessorTests(void)
{
}
...
void FilterProcessorTests::EnvelopeIntersect()
{
    ...
    try
    {
        ...
        FdoStringP className = UnitTestUtil::GetFeatureClass(L"UT_PROCESSSPATIALCONDITION", L"GEOM");
        selectcomm->SetFeatureClassName(className);

        selectcomm->SetFilter(L"GEOM ENVELOPEINTERSECTS GEOMFROMTEXT ('POLYGON ((5 5, 15 5, 15 15, 5 15, 5 5))')");
        ...
        if (count != 1)
            CPPUNIT_FAIL("Should be only 1 feature");
        reader->Close();

        selectcomm->SetFilter(L"GEOM ENVELOPEINTERSECTS GEOMFROMTEXT ('POLYGON ((55 55, 60 55, 60 60, 55 60, 55 55))')");
        ...
        if (count)
            CPPUNIT_FAIL("Should be zero feature");
        reader->Close();

        selectcomm->SetFilter(L"GEOM ENVELOPEINTERSECTS GEOMFROMTEXT ('POLYGON ((5 5, 105 5, 105 105, 5 105, 5 5))')");
        reader = selectcomm->Execute();
        count = 0;
        while (reader->ReadNext())
        {
        ...
        if (count != 2)
            CPPUNIT_FAIL("Should be only 2 features");
        reader->Close();

        selectcomm->SetFilter(L"GEOM ENVELOPEINTERSECTS GEOMFROMTEXT ('POLYGON ((5 5, 105 5, 105 105, 5 105, 5 5))') AND fid=1");
        reader = selectcomm->Execute();
        count = 0;
        while (reader->ReadNext())
        {
        ...
        if (count != 1)
            CPPUNIT_FAIL("Should be only 1 features");
        reader->Close();
...
void FilterProcessorTests::ProcessSpatialCondition()
{
    FdoPtr<FdoIConnection> conn = UnitTestUtil::CreateDefaultConnection();
    ASSERT_CAN_CONNECT(conn)
...
void FilterProcessorTests::ProcessSpatialConditionSRID()
{
    FdoPtr<FdoIConnection> conn = UnitTestUtil::CreateDefaultConnection();
    ASSERT_CAN_CONNECT(conn)
    ...
    {
        FdoPtr<FdoISelect> selectcomm = (FdoISelect *)conn->CreateCommand(FdoCommandType_Select);
        FdoStringP className = UnitTestUtil::GetFeatureClass(L"UT_SPATIALSRID", L"GEOM");
        selectcomm->SetFeatureClassName(className);
