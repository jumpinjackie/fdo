...
#ifndef ARCSDEAPPLYSCHEMACOMMAND_H
#define ARCSDEAPPLYSCHEMACOMMAND_H

#ifdef _WIN32
#pragma once
#endif // _WIN32

class ArcSDEApplySchemaCommand :
    public ArcSDECommand<FdoIApplySchema>
{
    friend class ArcSDEConnection;

    FdoFeatureSchema* mSchema;
...
private:
    ...
    ArcSDEApplySchemaCommand (FdoIConnection* connection);
...
protected:
    ...
    void delete_class (FdoFeatureSchema* schema, FdoClassDefinition* cls);
    void modify_class (FdoFeatureSchema* schema, FdoClassDefinition* cls);
    void add_schema (FdoFeatureSchema* schema);
    void delete_schema (FdoFeatureSchema* schema);
    void modify_schema (FdoFeatureSchema* schema);
    void SetArcSDEMetadata(SE_METADATAINFO &metadata, const CHAR* classname, const CHAR* property, const CHAR* value, const CHAR* description = NULL);
    void SetArcSDEMetadata(SE_METADATAINFO &metadata, const CHAR* classname, const CHAR* property, const wchar_t* value, const CHAR* description = NULL);
    const CHAR* FDOTypeToMetadataValue(FdoDataType dataType);
    void CleanUpIndexes(SE_INDEX_DEF* newIndexes, LONG numNewIndexes);
