...
void SltMetadata::BuildMetadataInfo(SltConnection* conn, SltStringList* lst)
{
    ...
    for(std::vector<Table*>::iterator itt = rtables.begin(); itt != rtables.end(); itt++)
    {
        ...
        for (int i=0; i<pTable->nCol; i++)
        {
            ...
            dt = ConvertDataType(pTable->aCol[i].affinity);
            ...
            if (propDefProc == NULL)
            {
                ...
                if (i == pTable->iPKey && ((pTable->aCol[i].colFlags & COLFLAG_PRIMKEY) > 0))
                ...
                if ((pTable->aCol[i].colFlags & COLFLAG_PRIMKEY) > 0)
                {
                    if (i == pTable->iPKey)
        ...
        if (pTable->pSelect == NULL && pTable->pIndex != NULL)
        {
            ...
            while (pIndex != NULL)
            {
                if (pIndex->nColumn != 0 && pIndex->idxType > 0)
                {
                    ...
                    {
                        i16 colIndex = pIndex->aiColumn[i];
                        if (XN_ROWID == colIndex)
                        {
                            //Skip the RowID column
                            continue;
                        }
                        FdoPtr<FdoPropertyDefinition> constrProp = clsProps->FindItem(A2W_SLOW(pTable->aCol[colIndex].zName).c_str());
...
bool SltMetadata::ExtractConstraints(ExprList * node, std::vector<SQLiteExpression>& result)
{
    int i;
    bool valid = true;
    ...
    if (NULL != node)
    {
        for (i = node->nExpr, pItem = node->a; i>0; i--, pItem++)
        {
            valid &= ExtractConstraints(pItem->pExpr, result);
        }
    }
    return valid;
}
