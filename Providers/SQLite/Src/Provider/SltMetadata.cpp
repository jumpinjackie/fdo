...
FdoClassDefinition* SltMetadata::ToClass()
{
        ...
        else
        {
            ...
            if (m_bUseFdoMetadata)
            {
                ...
                if (pfdostmt == NULL)
                {
                    ...
                    {
                        sqlite3_bind_text(pfdostmt, 1, m_table->zName, -1, SQLITE_STATIC);
            ...
            else if (m_table->pSelect != NULL) // is view
            {
    ...
    if (m_table->nCol && m_table->pSelect != NULL) // is view
    {
        ...
        ExtractViewDetailsInfo(sources, properties, expressions);
    }
...
void SltMetadata::ExtractViewDetailsInfo(StlMapNamesList& sources, StlMapNamesList& properties, StlMapNamesList& expressions)
{
    if(m_table->pSelect == NULL || m_table->pSelect->pSrc == NULL || m_table->pSelect->pSrc->nSrc == 0 || 
        m_table->pSelect->pEList == NULL || m_table->pSelect->pEList->nExpr == 0 || m_table->pSelect->op != TK_SELECT)
        return;
    
    for(int idx = 0; idx < m_table->pSelect->pSrc->nSrc; idx++)
    {
        const char* alias = m_table->pSelect->pSrc->a[idx].zAlias;
        const char* name = m_table->pSelect->pSrc->a[idx].zName;
        if (name == NULL)
            continue;
        sources.push_back(std::make_pair(name, ((alias == NULL) ? "" : name)));
    }
    for(int idx = 0; idx < m_table->pSelect->pEList->nExpr; idx++)
    {
        const char* name = m_table->pSelect->pEList->a[idx].zName;
        std::string value;
        if (m_table->pSelect->pEList->a[idx].pExpr->span.n)
            value = std::string((const char*)m_table->pSelect->pEList->a[idx].pExpr->span.z, m_table->pSelect->pEList->a[idx].pExpr->span.n);
        ...
        if (name != NULL)
        {
            if (m_table->pSelect->pEList->a[idx].pExpr->op == TK_DOT)
