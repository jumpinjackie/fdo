...
void RfpTestEcw::testRGB()
{
	...
	FdoPtr<FdoIoStream> stream = FdoIoFileStream::Create(L"../../TestData/Ecw/golf_course.xml", L"r");
	...
	CPPUNIT_ASSERT(xSize == 5591);
	CPPUNIT_ASSERT(ySize == 5156);
	...
	int numTileRows = (5157 - 1) / tileSizeX + 1;
	int numTileCols = (5592 - 1) / tileSizeY + 1;
	...
	raster->SetImageXSize(1397);
	raster->SetImageYSize(1289);
	...
	// iterate all rows
	FdoByte buff[5592]; //5592=1397*4+4
	for (int row = 0; row < 1289; row++)
	{
		FdoInt32 numRead = reader->ReadNext(buff, 0, 5592);
		CPPUNIT_ASSERT(numRead == 5592);
	}
