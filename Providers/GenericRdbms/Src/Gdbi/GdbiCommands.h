...
typedef struct {
    int     cursor;
    char    seq_name[RDBI_TABLE_NAME_SIZE];
    int     next;
    int     size;
...
public:
    ...
    int run_sql( char *sql,  bool isDDL, int *rows_processed = NULL  );

	int run_sql( wchar_t *sql,  bool isDDL, int *rows_processed = NULL  );


	int sql( wchar_t *sql,  int *qid  );
    ...
    long NextSequenceNumber( char* sequence );
