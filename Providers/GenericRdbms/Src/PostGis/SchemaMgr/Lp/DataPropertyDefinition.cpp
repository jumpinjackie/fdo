...
void FdoSmLpPostGisDataPropertyDefinition::Finalize()
{
    ...
    if (state == FdoSmObjectState_Initial)  // need to check this to avoid infinite loop with call to GetColumn()
    {
        ...
        if ( defaultLen > 0)
        {
            //TODO - Revisit default value handling. Currently weeding out ones
            //not liked by the SQLite provider.
            if ( (defaultValue.Contains(L":") || defaultValue.Contains(L"(")) && ! defaultValue.Contains(L"TIMESTAMP") ) 
                SetDefaultValue( FdoPtr<FdoDataValue>() );
        }
    }
}
