...
#define			CLASS_NAME			L"CData"
#define			CLASS_NAME_BASE		L"CDataBase"
...
FdoPropertyValue* FdoConstraintsTest::AddNewProperty( FdoPropertyValueCollection* propertyValues, const wchar_t *name )
{
...
void FdoConstraintsTest::TestDescribeConstraints ()
{
	...
	try
	{
		...
		DescribeConstraintsSchema( connection );
...
void FdoConstraintsTest::DescribeConstraintsSchema( FdoIConnection* connection )
{
	FdoPtr<FdoISchemaCapabilities>	schemaCap = connection->GetSchemaCapabilities();
	...
	CPPUNIT_ASSERT_MESSAGE("Wrong number of unique keys", count == NUM_UNIQUE_KEYS );
	...
	CPPUNIT_ASSERT_MESSAGE("Wrong number of CHECK constraints", count == NUM_CHECK_CONSTRAINTS );
}
