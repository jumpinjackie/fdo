...
//FdoPtr<FdoISQLCommand> sqlCmd;
//sqlCmd = (FdoISQLCommand*)mConnection->CreateCommand( FdoCommandType_SQLCommand );
//sqlCmd->SetSQLStatement( L"update [dbo].[myclass] set STEPS_COMPLETED = :MatchedFeatures where STEPS_COMPLETED > :MatchedFeatures and STEPS_COMPLETED < :OldConflictsResolved" );
//FdoPtr<FdoParameterValueCollection> pvc = sqlCmd->GetParameterValues();
//
//FdoPtr<FdoInt32Value> v1 = FdoInt32Value::Create(2);
//FdoPtr<FdoParameterValue> pv1 = FdoParameterValue::Create(L"MatchedFeatures", v1);
//pvc->Add(pv1);
//
//FdoPtr<FdoInt32Value> v2 = FdoInt32Value::Create(5);
//FdoPtr<FdoParameterValue> pv2 = FdoParameterValue::Create(L"OldConflictsResolved", v2);
//pvc->Add(pv2);
//
//int vch = sqlCmd->ExecuteNonQuery();

void SqlServerFdoSqlCmdTest::Romy()
{
    try
    {
        //FdoPtr<FdoIGetClassNames>  pDescSchemaCmd = (FdoIGetClassNames*) mConnection->CreateCommand(FdoCommandType_GetClassNames);
        ////pDescSchemaCmd->SetSchemaName(L"TbSchema");
        //FdoPtr<FdoStringCollection> schName = pDescSchemaCmd->Execute();

        FdoPtr<FdoIGetSchemaNames>  pSchCmdCmd = (FdoIGetSchemaNames*) mConnection->CreateCommand(FdoCommandType_GetSchemaNames);
        FdoPtr<FdoStringCollection> schName = pSchCmdCmd->Execute();

        FdoPtr<FdoIDescribeSchema>  pDescSchemaCmd = (FdoIDescribeSchema*) mConnection->CreateCommand(FdoCommandType_DescribeSchema);
        //FdoPtr<FdoStringCollection> col = FdoStringCollection::Create();
        //col->Add(L"TbSchema:WA_V_C_CABLE");
        //col->Add(L"TbSchema:WA_V_ARMATURE");
        //col->Add(L"TbSchema:CONSTRUCT_LINES");
        //pDescSchemaCmd->SetClassNames(col);
        //pDescSchemaCmd->SetSchemaName(L"TbSchema");
        //clock_t start = clock();
        FdoFeatureSchemasP          fsc = pDescSchemaCmd->Execute();
        FdoPtr<FdoFeatureSchema> dch = fsc->GetItem(0);
        FdoPtr<FdoClassCollection> clss = dch->GetClasses();
        FdoPtr<FdoClassDefinition> cls = clss->GetItem(L"WA_V_ARMATURE");
        FdoPtr<FdoPropertyDefinitionCollection> prop = cls->GetProperties();
        for (int idx = 0; idx < prop->GetCount(); idx++)
        {
            FdoPtr<FdoPropertyDefinition> pr = prop->GetItem(idx);
            FdoString* name = pr->GetName();
            if (pr->GetPropertyType() == FdoPropertyType_DataProperty)
            {
                FdoDataPropertyDefinition* pd = static_cast<FdoDataPropertyDefinition*>(pr.p);
                printf ("\n%ls - readonly=%d", name, (int)pd->GetReadOnly());
            }
            else if (pr->GetPropertyType() == FdoPropertyType_GeometricProperty)
            {
                FdoGeometricPropertyDefinition* pg = static_cast<FdoGeometricPropertyDefinition*>(pr.p);
                printf ("\n%ls - readonly=%d", name, (int)pg->GetReadOnly());
            }
        }

        //clock_t finish = clock();
        //double duration = (double)(finish - start) / CLOCKS_PER_SEC;
        //printf ("\nDescribeSchema execution in %5.3f secs.\n", duration);
    }
    catch( FdoException *ex )
    {
		TestCommonFail (ex);
...
//e:\trunk\Utilities\SchemaMgr\Src\Sm\Ph\Owner.cpp
//Line 1277 = dbObject->CacheBaseObjects( baseObjectReader );
//
//e:\trunk\Utilities\SchemaMgr\Src\Sm\Ph\DbObject.cpp
//Line 950 = FdoSmPhDbObject::CacheBaseObjects
//
//
//e:\trunk\Providers\GenericRdbms\Src\SchemaMgr\Ph\Rd\QueryReader.cpp
//Line 62 = bool FdoSmPhRdGrdQueryReader::ReadNext()
//
//e:\trunk\Utilities\SchemaMgr\Src\Sm\Ph\Reader.cpp
//Line 57 = FdoSmPhReader::ReadNext()
//
//e:\trunk\Utilities\SchemaMgr\Src\Sm\Ph\GroupReader.cpp
//Line 33 = FdoSmPhGroupReader::ReadNext()
//
//
//
//
//
//FdoSmPhGroupReader::GetString
//FdoSmPhRdSqsBaseObjectReader::GetString
//FdoSmPhRdBaseObjectReader::GetString
//FdoSmPhReadWrite::GetString
//FdoSmPhReadWrite::GetField
//FdoSmPhFieldP GetField
//FdoSmPhRdGrdQueryReader::GetString
//FdoSmPhRdSqsBaseObjectReader::GetString
