...
if test "$SHOWHELP" == yes; then
   ...
   rm -rf "/usr/local/fdo-4.0.0"
   rm -rf "$FDOTARGETFOLDER"

   mkdir -p "$FDOTARGETFOLDER"
   echo "*start copy zip files"
   cp --force "$FDOSOURCETARGZ/fdo-4.0.0_$FDOBUILDNUMBERRTB.tar.gz" "$FDOTARGETFOLDER"
   cp --force "$FDOSOURCETARGZ/fdoshp-4.0.0_$FDOBUILDNUMBERRTB.tar.gz" "$FDOTARGETFOLDER"
   cp --force "$FDOSOURCETARGZ/fdosdf-4.0.0_$FDOBUILDNUMBERRTB.tar.gz" "$FDOTARGETFOLDER"
   cp --force "$FDOSOURCETARGZ/fdowfs-4.0.0_$FDOBUILDNUMBERRTB.tar.gz" "$FDOTARGETFOLDER"
   cp --force "$FDOSOURCETARGZ/fdowms-4.0.0_$FDOBUILDNUMBERRTB.tar.gz" "$FDOTARGETFOLDER"
   cp --force "$FDOSOURCETARGZ/fdoarcsde-4.0.0_$FDOBUILDNUMBERRTB.tar.gz" "$FDOTARGETFOLDER"
   cp --force "$FDOSOURCETARGZ/fdordbms-4.0.0_$FDOBUILDNUMBERRTB.tar.gz" "$FDOTARGETFOLDER"
   cp --force "$FDOSOURCETARGZ/fdogdal-4.0.0_$FDOBUILDNUMBERRTB.tar.gz" "$FDOTARGETFOLDER"
   echo "copy zip files done..."

   echo "*start unzip files"
   cd "$FDOTARGETFOLDER"
   gzip -dN "fdo-4.0.0_$FDOBUILDNUMBERRTB.tar.gz"
   gzip -dN "fdoshp-4.0.0_$FDOBUILDNUMBERRTB.tar.gz"
   gzip -dN "fdosdf-4.0.0_$FDOBUILDNUMBERRTB.tar.gz"
   gzip -dN "fdowfs-4.0.0_$FDOBUILDNUMBERRTB.tar.gz"
   gzip -dN "fdowms-4.0.0_$FDOBUILDNUMBERRTB.tar.gz"
   gzip -dN "fdoarcsde-4.0.0_$FDOBUILDNUMBERRTB.tar.gz"
   gzip -dN "fdordbms-4.0.0_$FDOBUILDNUMBERRTB.tar.gz"
   gzip -dN "fdogdal-4.0.0_$FDOBUILDNUMBERRTB.tar.gz"

   tar -xf "fdo-4.0.0_$FDOBUILDNUMBERRTB.tar"
   tar -xf "fdoshp-4.0.0_$FDOBUILDNUMBERRTB.tar"
   tar -xf "fdosdf-4.0.0_$FDOBUILDNUMBERRTB.tar"
   tar -xf "fdowfs-4.0.0_$FDOBUILDNUMBERRTB.tar"
   tar -xf "fdowms-4.0.0_$FDOBUILDNUMBERRTB.tar"
   tar -xf "fdoarcsde-4.0.0_$FDOBUILDNUMBERRTB.tar"
   tar -xf "fdordbms-4.0.0_$FDOBUILDNUMBERRTB.tar"
   tar -xf "fdogdal-4.0.0_$FDOBUILDNUMBERRTB.tar"
   popd >& /dev/null
   ...
   mkdir -p "/usr/local/fdo-4.0.0/lib"
   export LD_LIBRARY_PATH=$LD_LIBRARY_PATH:/usr/local/fdo-4.0.0/lib:$SDEHOME/lib
   popd >& /dev/null
