...
if test "$SHOWHELP" == yes; then
   ...
   rm -rf "/usr/local/fdo-3.5.0"
   rm -rf "$FDOTARGETFOLDER"

   mkdir -p "$FDOTARGETFOLDER"
   echo "*start copy zip files"
   cp --force "$FDOSOURCETARGZ/fdo-3.5.0_$FDOBUILDNUMBERRTB.tar.gz" "$FDOTARGETFOLDER"
   cp --force "$FDOSOURCETARGZ/fdoshp-3.5.0_$FDOBUILDNUMBERRTB.tar.gz" "$FDOTARGETFOLDER"
   cp --force "$FDOSOURCETARGZ/fdosdf-3.5.0_$FDOBUILDNUMBERRTB.tar.gz" "$FDOTARGETFOLDER"
   cp --force "$FDOSOURCETARGZ/fdowfs-3.5.0_$FDOBUILDNUMBERRTB.tar.gz" "$FDOTARGETFOLDER"
   cp --force "$FDOSOURCETARGZ/fdowms-3.5.0_$FDOBUILDNUMBERRTB.tar.gz" "$FDOTARGETFOLDER"
   cp --force "$FDOSOURCETARGZ/fdoarcsde-3.5.0_$FDOBUILDNUMBERRTB.tar.gz" "$FDOTARGETFOLDER"
   cp --force "$FDOSOURCETARGZ/fdordbms-3.5.0_$FDOBUILDNUMBERRTB.tar.gz" "$FDOTARGETFOLDER"
   cp --force "$FDOSOURCETARGZ/fdogdal-3.5.0_$FDOBUILDNUMBERRTB.tar.gz" "$FDOTARGETFOLDER"
   echo "copy zip files done..."

   echo "*start unzip files"
   cd "$FDOTARGETFOLDER"
   gzip -dN "fdo-3.5.0_$FDOBUILDNUMBERRTB.tar.gz"
   gzip -dN "fdoshp-3.5.0_$FDOBUILDNUMBERRTB.tar.gz"
   gzip -dN "fdosdf-3.5.0_$FDOBUILDNUMBERRTB.tar.gz"
   gzip -dN "fdowfs-3.5.0_$FDOBUILDNUMBERRTB.tar.gz"
   gzip -dN "fdowms-3.5.0_$FDOBUILDNUMBERRTB.tar.gz"
   gzip -dN "fdoarcsde-3.5.0_$FDOBUILDNUMBERRTB.tar.gz"
   gzip -dN "fdordbms-3.5.0_$FDOBUILDNUMBERRTB.tar.gz"
   gzip -dN "fdogdal-3.5.0_$FDOBUILDNUMBERRTB.tar.gz"

   tar -xf "fdo-3.5.0_$FDOBUILDNUMBERRTB.tar"
   tar -xf "fdoshp-3.5.0_$FDOBUILDNUMBERRTB.tar"
   tar -xf "fdosdf-3.5.0_$FDOBUILDNUMBERRTB.tar"
   tar -xf "fdowfs-3.5.0_$FDOBUILDNUMBERRTB.tar"
   tar -xf "fdowms-3.5.0_$FDOBUILDNUMBERRTB.tar"
   tar -xf "fdoarcsde-3.5.0_$FDOBUILDNUMBERRTB.tar"
   tar -xf "fdordbms-3.5.0_$FDOBUILDNUMBERRTB.tar"
   tar -xf "fdogdal-3.5.0_$FDOBUILDNUMBERRTB.tar"
   popd >& /dev/null
