...
public:
  ...
  int m_IdsGeometryType;
  
  int m_SRID;
  wstring m_SrText; // Wkt
  //simon
  bool m_HasElevation;
  bool m_HasMeasureElevation;
};
...
// Stream creating
  ...
  static bool SetStreamDataGeometryWkt(otl_stream* Stream,int SqlParamNum,FdoByteArray* GeometryFgf);
  
  
  void ReadGeometryColumns(vector<c_KgInf_GeomteryColumns>& GeomData);
  void QueryTableGeometry(c_KgInf_GeomteryColumns& TableDesc);
};
