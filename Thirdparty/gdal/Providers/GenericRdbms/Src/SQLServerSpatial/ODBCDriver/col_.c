...
static const char * typeNumberToName(odbcdr_DriverType driver_type, size_t odbcType, size_t odbcTypeDateTimeSubcode, const char * odbcTypeName)
{
    size_t i;
    size_t array_size = sizeof(typeNameMap_S) / sizeof(typeNameMap_S[0]);
...
static const wchar_t * typeNumberToNameW(odbcdr_DriverType driver_type, size_t odbcType, size_t odbcTypeDateTimeSubcode, const wchar_t * odbcTypeName)
{
    size_t i;
    size_t array_size = sizeof(typeNameMap_S) / sizeof(typeNameMap_S[0]);
    ...
    for (position=1;  position <= num_cols;  position++)
    {
        *szColumnName.wString = L'\0';
        ...
        if (context->odbcdr_UseUnicode)
        {
            ...
            (void) wcscpy(newNle.typeW, typeNumberToNameW(connData->driver_type, ssDataType2, ssDataTypeDateTimeSubcode, NULL));
        ...
        else
        {
            ...
            if (ret != ODBCDR_SUCCESS)
                ssDataTypeDateTimeSubcode = -1; /* Some drivers do not support this subcode. */
            (void) strcpy(newNle.type, typeNumberToName(connData->driver_type,ssDataType2, ssDataTypeDateTimeSubcode, NULL));
        ...
        *szRemarks.wString = L'\0';
        ...
        if (!found)
        {
            ...
            if (context->odbcdr_UseUnicode)
            {
                ...
                wcscpy(newNle.typeW, typeNumberToNameW(connData->driver_type, ssDataType, -1, szTypeName.cwString));
            }
            else
            {
                strcpy(newNle.name, szColumnName.ccString);
                strcpy(newNle.type, typeNumberToName(connData->driver_type, ssDataType, -1, szTypeName.ccString));
            }
	...
	)
{
	...
	*eof = FALSE;
    ...
    else
    {
        ...
        *length = (int)nle->length;
        *scale = (int)nle->scale;
        *nullable = (int)nle->isnullable;
        *position = (int)nle->position;
        *is_autoincrement = (int)nle->isautogenerated;
    }
