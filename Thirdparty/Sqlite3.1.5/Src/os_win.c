...
#define  SETUP_CONVERSION  \
	size_t	   _size

#define  UTF2W( zStr ) (\
	( zStr == NULL) ? NULL : (\
		_size = (lstrlenA(zStr)+1),\
		(INT_MAX/2<_size)? NULL :  \
		utf2wide(zStr, _size, (LPWSTR) alloca(_size*sizeof(WCHAR)), _size )))

#define  W2UTF( zStr ) (\
	( zStr == NULL) ? NULL : (\
		_size = (lstrlenW(zStr)+1),\
		(INT_MAX/10<_size)? NULL :  \
		wide2utf(zStr, _size, (char*) alloca(_size*sizeof(char)*5), _size*5 )))
...
/*
** Helper function to convert from UTF8 to wide char
*/
wchar_t* utf2wide( char* inStr, size_t inSize, wchar_t* outStr, size_t outSize )
{
	if( 0 == MultiByteToWideChar ( CP_UTF8, 0, inStr, inSize, outStr, outSize) )
		return NULL;
...
** Helper function to convert from UTF8 to wide char
*/
char* wide2utf( wchar_t* inStr, size_t inSize,  char* outStr, size_t outSize )
{
	if( 0 == WideCharToMultiByte ( CP_UTF8, 0, inStr, inSize, outStr, outSize, NULL, NULL) )
		return NULL;
...
int sqlite3OsFileExists(const char *zFilename){
  SETUP_CONVERSION;
  return GetFileAttributesW(UTF2W(zFilename)) != 0xffffffff;
}
...
char *sqlite3OsFullPathname(const char *zRelative){
  ...
  char *ztmpFull;
  int nByte;
  wchar_t* zwRelative = UTF2W(zRelative);
  nByte = GetFullPathNameW(zwRelative, 0, 0, &zNotUsed) + 1;
  zwFull = (wchar_t *)alloca( nByte*sizeof(wchar_t) );
  if( zwFull==0 ) return 0;
  GetFullPathNameW(zwRelative, nByte, zwFull, &zNotUsed);
  ztmpFull = W2UTF( zwFull );
  zFull = sqliteMalloc( strlen(ztmpFull)+1 );
  if( zFull==0 ) return 0;
  strcpy(zFull,ztmpFull);
  return zFull;
}
